...
- **Any command that updates `.claude/skills/`** → spawn a **background sub-agent** for `/t1k:sync-back` (`Task` tool, `run_in_background: true`). NEVER manually copy files and NEVER run the skill inline. See `rules/error-recovery.md` → "Background Sub-Agent Invocation".
