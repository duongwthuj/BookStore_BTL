...
4. **Routing — follow `rules/routing-protocol.md`**
5. **Skill Activation — follow `rules/activation-protocol.md`**
6. **Module Context — follow `rules/subagent-injection-protocol.md`** (if spawning agents)
...
- **After any skill file updated**: ALWAYS invoke `/t1k:sync-back` as a **background sub-agent** (`Task` tool with `run_in_background: true`). NEVER run inline — it interrupts the user's current task. See `rules/error-recovery.md` → "Background Sub-Agent Invocation" for the spawn pattern.
...
5. **Sync-back check**: if any `$HOME/.claude/skills/` files modified, spawn a **background sub-agent** to run `/t1k:sync-back` (see `rules/error-recovery.md` → "Background Sub-Agent Invocation"). NEVER manually copy files and NEVER run the skill inline.
